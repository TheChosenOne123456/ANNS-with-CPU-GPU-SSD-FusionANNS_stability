...
#pragma region K-NN search
        ...
        */
        ...
        ErrorCode Index<T>::SearchPQIndex_CPU(QueryResult &p_query, std::shared_ptr<VectorSet> vectorSet, int PQVectorsCount, int PQVectorDim, std::vector<int>& numVecPerPostinglist, std::vector<std::unique_ptr<int[]>>& postinglist, std::unordered_set<int>& postingIDSet, SearchStats *p_stats) const
        {
            ...
            // 鎼滅储閫昏緫
            // double vectorMilliseconds = 0;
            // double vectorElapsedMilliseconds = 0;
            // double findTableElapsedMilliseconds = 0;
            // double addPointElapsedMilliseconds = 0;
            ...
            for (uint32_t pi = 0; pi < postingListCount; ++pi)
            {
                ...
                for (int vi = 0; vi < numvec; vi++)
                {
                    ...
                    // auto findTableStartTime = std::chrono::high_resolution_clock::now();
                    // 鏌ヨ〃璁＄畻缁撴灉
                    float dist = L2Distance_Dmul4(reinterpret_cast<const uint8_t *>(queryResults->GetnewPQTarget()), vector, PQVectorDim);
                    
                    // auto addPointStartTime = std::chrono::high_resolution_clock::now();
                    queryResults->AddPoint(vectorID, dist);
                    // auto addPointEndTime = std::chrono::high_resolution_clock::now();
                    // std::chrono::duration<double, std::milli> vectorelapsed0 = vectorMiddleTime - vectorStartTime;
                    // std::chrono::duration<double, std::milli> vectorelapsed1 = findTableStartTime - vectorMiddleTime;
                    // std::chrono::duration<double, std::milli> findelapsed = addPointStartTime - findTableStartTime;
                    // std::chrono::duration<double, std::milli> addelapsed = addPointEndTime - addPointStartTime;
                    // vectorMilliseconds += vectorelapsed0.count();
                    // vectorElapsedMilliseconds += vectorelapsed1.count();
                    // findTableElapsedMilliseconds += findelapsed.count();
                    // addPointElapsedMilliseconds += addelapsed.count();
                }
            }
            // p_stats->vectorLatency0 = vectorMilliseconds;
            // p_stats->vectorLatency1 = vectorElapsedMilliseconds;
            // p_stats->findTableLatency = findTableElapsedMilliseconds;
            // p_stats->addPointLatency = addPointElapsedMilliseconds;
            auto sortStartTime = std::chrono::high_resolution_clock::now();
            // 鍫嗘帓搴忥紝鍒╃敤鍫嗘帓搴忥紝灏唌_result鎺掑簭涓轰粠灏忔墦鍒扮殑椤哄簭
        ...
        /*
            绗竴涓狦PU鐗堟湰锛屾暟鎹紶杈撴棤蹇у寲
        */
        template <typename T>
        ErrorCode Index<T>::SearchPQIndex_with_GPU(QueryResult &p_query, std::shared_ptr<VectorSet> vectorSet, int PQVectorsCount, int PQVectorDim,
                                                   std::vector<int>& numVecPerPostinglist, std::vector<std::unique_ptr<int[]>>& postinglist, void *d_PQVectorSet, SearchStats *p_stats) const
        {
            auto cutStartTime = std::chrono::high_resolution_clock::now();
            COMMON::QueryResultSet<T> *queryResults = (COMMON::QueryResultSet<T> *)&p_query;
            ...
            uint8_t *d_table;
            cudaMalloc((void **)&d_table, 256 * PQVectorDim * sizeof(float));
            cudaMemcpy(d_table, h_table, 256 * PQVectorDim * sizeof(float), cudaMemcpyHostToDevice);
            // std::vector<float> dist;
            for (uint32_t pi = 1; pi <= postingListCount; ++pi)
            {

                int postingID = postingIDs[pi - 1];
                int numvec = numVecPerPostinglist[postingID];
            ...
            int *d_vectorIDs;
            cudaMalloc((void **)&d_vectorIDs, sizeof(int) * numVector);
            cudaMemcpy(d_vectorIDs, vectorIDs.data(), sizeof(int) * numVector, cudaMemcpyHostToDevice);
            // GPU澶勭悊浠ｇ爜
            std::vector<float> dist(numVector);
            // processFunction(d_PQVectorSet, d_vectorIDs, d_table, dist, PQVectorDim, numVector * PQVectorDim);
            // processFunction(d_PQVectorSet, d_vectorIDs, d_table, dist, PQVectorDim, numVector);
            processFunction1(d_PQVectorSet, d_vectorIDs, d_table, dist, PQVectorDim, numVector);

            for (int i = 0; i < numVector; i++)
                queryResults->AddPoint(vectorIDs[i], dist[i]);
            // 鍫嗘帓搴忥紝鍒╃敤鍫嗘帓搴忥紝灏唌_result鎺掑簭涓轰粠灏忔墦鍒扮殑椤哄簭
            queryResults->SortResult();
            return ErrorCode::Success;
        }
        ...
        */
        template <typename T>
        ErrorCode Index<T>::SearchPQIndex_GPU(QueryResult& p_query, int PQVectorsCount, int PQVectorDim, 
        ...
        std::unordered_set<int>& postingIDSet, SearchStats* p_stats) const
        {
            ...
            // COMMON::BloomFilter deduper(299993, 7);
            // for (uint32_t pi = 1; pi <= postingListCount; ++pi)
            // {

            //     int postingID = postingIDs[pi - 1];
            //     _mm_prefetch((const char*)postinglist[postingID], _MM_HINT_T0);
            //     int numvec = numVecPerPostinglist[postingID];
            //     for (int vi = 0; vi < numvec; vi++)
            //     {
            //         int vectorID = postinglist[postingID][vi];
            //         // if (deduper.possiblyContains(vectorID))
            //         //     continue;
            //         // deduper.add(vectorID);
            //         vectorIDs.push_back(vectorID);
            //     }
            // }          
