...
float DistanceUtils::PQADCDistance_AVX512(const std::uint8_t* pX, const std::uint8_t* pY, DimensionType length)
    // pX must be query distance table for ADC
{
    float out = 0;        
    float* ptr = (float*)pX;
    int remainingDim = length;
    while (remainingDim >= 16) {
        __m512 sum512 = _mm512_setzero_ps();
        for (int i = 0; i < 16; i++) {
            __m512 x512 = _mm512_loadu_ps(ptr + static_cast<int>(pY[i]));
            sum512 = _mm512_add_ps(sum512, x512);
        }
        __attribute__((aligned(64))) float sum[16];
        _mm512_store_ps(sum, sum512);
    ...
    // Calculate remaining elements using scalar computation
    for (int i = 0; i < remainingDim; i++) {
        out += ptr[static_cast<int>(pY[i])];
        ptr += 256; // m_KsPerSubvector
    }
