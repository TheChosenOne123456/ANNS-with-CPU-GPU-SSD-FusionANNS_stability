...
inline float warmup_ip_x0_q(
...
#if defined(__AVX512F__) && defined(__AVX512VL__) && defined(__AVX512VPOPCNTDQ__)
    ...
    for (size_t i = 0; i < vec_end; i += vec_width) {
        for (size_t k = 0; k < vec_width; k++) {
            const uint64_t x = data[i + k];
            ppc_scalar += __builtin_popcountll(x);
            for (uint32_t j = 0; j < b_query; j++) {
                ip_scalar += __builtin_popcountll(x & query[(i + k) * b_query + j]) << j;
            }
        }
    }
#endif
